...
function App() {
  ...
  return (
    <div className="App">
        ...
        <div className="list">
          <h3 onClick={()=>{ setFlag(true)}} >{ title[0]} 
        ...
        <div className="list">
        <h3 onClick={()=>{ onoff }} >{ title[1]} <span onClick={()=>{setLike(like + 1)}}>❤</span>  {like[1]} </h3>
          <span>3월 31일 발행</span>
          <hr />
        </div>
        <div className="list">
        <h3 onClick={()=>{ setFlag(true)}} >{ title[2]} <span onClick={()=>{setLike(like + 1)}}>❤</span>  {like[2]} </h3>
          <span>3월 31일 발행</span>
          <hr />
        </div>
        <button onClick={()=>{ setModal }}>MyModal ON/OFF</button>
